...
socket.on("data-server-5d", function (msg) {
    ...
    if (msg) {
        ...
        if (checkData == msg.game) {
            ...
            function checkRoundResult5D(attempt) {
                attempt = attempt || 1;
                $.ajax({
                    type: "POST",
                        ...
                        if (Result && data.length > 0) {
                            ...
                            if (roundBets.length > 0) {
                                ...
                                if (hasPending && attempt < 6) {
                                    setTimeout(function() { checkRoundResult5D(attempt + 1); }, 700);
                                    return;
                                }
                                ...
                                if (typeof window.showFloatingToast === 'function') {
                                    if (isWin && totalWin > 0) {
                                        window.showFloatingToast('win', { amount: totalWin, period: endedPeriod, game: '5D Lotre' });
                                    } else if (totalLoss > 0) {
                                        window.showFloatingToast('loss', { amount: totalLoss, period: endedPeriod, game: '5D Lotre' });
                                    }
                                }
                            }
                        }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (window.checkAndShowGameResult && data && data.length > 0 && data[0].status != 0) {
                window.checkAndShowGameResult('5d' + $('html').attr('data-dpr'), data[0].stage, data[0].result, data);
            }
        },
    });
}
