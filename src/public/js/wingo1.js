          ...
          if (msg.data && msg.data[1] && data.length > 0) {
            ...
            if (roundBets.length > 0) {
              ...
              if (typeof window.showFloatingToast === 'function') {
                if (isWin && totalWin > 0) {
                  window.showFloatingToast('win', { amount: totalWin, period: endedPeriod, game: 'Win Go 1Min' });
                } else if (totalLoss > 0) {
                  window.showFloatingToast('loss', { amount: totalLoss, period: endedPeriod, game: 'Win Go 1Min' });
                }
              }
            }
          }
...
function displayGameResultNotice(list_orders) {
  ...
  if (betTime > 0 && (Date.now() - betTime > 240000)) {
  ...
  if (typeof window.showFloatingToast === 'function') {
    if (isWin && totalWin > 0) {
      window.showFloatingToast('win', { amount: totalWin, period: stage, game: 'Win Go 1Min' });
    } else if (totalLoss > 0) {
      window.showFloatingToast('loss', { amount: totalLoss, period: stage, game: 'Win Go 1Min' });
    }
  }
}
