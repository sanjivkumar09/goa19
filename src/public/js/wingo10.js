            ...
            if (msg.data && msg.data[1] && data.length > 0) {
              ...
              if (roundBets.length > 0) {
                ...
                if (typeof window.showFloatingToast === 'function') {
                  if (isWin && totalWin > 0) {
                    window.showFloatingToast('win', { amount: totalWin, period: endedPeriod, game: 'Win Go 10Min' });
                  } else if (totalLoss > 0) {
                    window.showFloatingToast('loss', { amount: totalLoss, period: endedPeriod, game: 'Win Go 10Min' });
                  }
                }
              }
            }
...
function displayGameResultNotice(list_orders) {
  ...
  if (betTime > 0 && (Date.now() - betTime > 900000)) {
  ...
  if (typeof window.showFloatingToast === 'function') {
    if (isWin && totalWin > 0) {
      window.showFloatingToast('win', { amount: totalWin, period: stage, game: 'Win Go 10Min' });
    } else if (totalLoss > 0) {
      window.showFloatingToast('loss', { amount: totalLoss, period: stage, game: 'Win Go 10Min' });
    }
  }
}
      ...
      success: function (response) {
        if (response && response.data && response.data.gameslist) {
          ...
          if (window.checkAndShowGameResult && data && data.length > 0 && data[0].status != 0) {
            window.checkAndShowGameResult('wingo10', data[0].stage, data[0].result, data);
          }
        }
      },
      complete: done,
