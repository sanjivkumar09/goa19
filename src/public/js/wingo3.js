            ...
            if (msg.data && msg.data[1] && data.length > 0) {
              ...
              if (roundBets.length > 0) {
                ...
                if (typeof window.showFloatingToast === 'function') {
                  if (isWin && totalWin > 0) {
                    window.showFloatingToast('win', { amount: totalWin, period: endedPeriod, game: 'Win Go 3Min' });
                  } else if (totalLoss > 0) {
                    window.showFloatingToast('loss', { amount: totalLoss, period: endedPeriod, game: 'Win Go 3Min' });
                  }
                }
              }
            }
...
function displayGameResultNotice(list_orders) {
  ...
  if (typeof window.showFloatingToast === 'function') {
    if (isWin && totalWin > 0) {
      window.showFloatingToast('win', { amount: totalWin, period: stage, game: 'Win Go 3Min' });
    } else if (totalLoss > 0) {
      window.showFloatingToast('loss', { amount: totalLoss, period: stage, game: 'Win Go 3Min' });
    }
  }
}
